...
class xdomain_mix(nn.Module):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(xdomain_mix, self).__init__()
        self.hparams = hparams
        ...
        self.class_target_layer = self.network.network.layer4
...
class xdomain_mix_sam(nn.Module):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(xdomain_mix_sam, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.hparams = hparams
        self.input_shape = input_shape
        self.num_classes = num_classes
        self.num_domains = num_domains
...
class DSU(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DSU, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.network = networks.DSU(input_shape, num_classes, hparams, uncertainty=0.5)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss = F.cross_entropy(self.network.forward(all_x, True), all_y)
    ...
    def predict(self, x):
        return self.network.forward(x, False)
        

class MixStyle_module(nn.Module):
...
class MixStyle(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.mix_module = MixStyle_module()

    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])

        loss = F.cross_entropy(self.train_mixstyle(all_x), all_y)
    ...
    def train_mixstyle(self, x):
        out = x
        out = self.network[0].network.conv1(out)
        out = self.network[0].network.bn1(out)
        out = self.network[0].network.relu(out)
        out = self.network[0].network.maxpool(out)
        out = self.network[0].network.layer1(out)
        out = self.mix_module(out)
        
        out = self.network[0].network.layer2(out)
        out = self.mix_module(out)
        
        out = self.network[0].network.layer3(out)
        out = self.network[0].network.layer4(out)
        out = self.network[0].network.avgpool(out)
...
class LISA(nn.Module):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.mix_alpha = hparams['mix_alpha']
    ...
    def mix_up(mix_alpha, x, y, x2=None, y2=None):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # random sample same domain samples in the data loader in the original implementation
        # treat one batch as two batches
        x1, y1, d1 = [], [], []
        x2, y2, d2 = [], [], []
        for i in range(self.batch_size*self.num_domains):
            idx = i % self.batch_size
            if idx < self.batch_size // 2:
                x1.append(all_x[i].unsqueeze(0))
                y1.append(all_y[i])
                d1.append(all_domain[i])
            else:
                x2.append(all_x[i].unsqueeze(0))
                y2.append(all_y[i])
                d2.append(all_domain[i])

        x1 = torch.cat(x1).to(all_x.device)
        x2 = torch.cat(x2).to(all_x.device)
        y1 = torch.stack(y1).reshape(-1).to(all_y.device)
        y2 = torch.stack(y2).reshape(-1).to(all_y.device)

        y1_onehot = torch.zeros(len(y1), self.num_classes).to(y1.device)
        y1 = y1_onehot.scatter_(1, y1.unsqueeze(1), 1)
        y2_onehot = torch.zeros(len(y2), self.num_classes).to(y2.device)
        ...
        if self.cutmix:
            ...
            bbx1, bby1, bbx2, bby2 = LISA.rand_bbox(input.size(), lam)
            input[:, :, bbx1:bbx2, bby1:bby2] = input[rand_index, :, bbx1:bbx2, bby1:bby2]
            lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (input.size()[-1] * input.size()[-2]))
            mixed_y = lam * target_a + (1 - lam) * target_b
            outputs = self.predict(input)
        else:
            mixed_x1, mixed_y1 = LISA.mix_up(self.mix_alpha, x1, y1, x2, y2)
            mixed_x2, mixed_y2 = LISA.mix_up(self.mix_alpha, x2, y2, x1, y1)
            mixed_x = torch.cat([mixed_x1, mixed_x2])
            mixed_y = torch.cat([mixed_y1, mixed_y2])
            outputs = self.predict(mixed_x)
...
class GroupDRO(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GroupDRO, self).__init__(input_shape, num_classes, num_domains,
                                        hparams)
        self.register_buffer("q", torch.Tensor(num_domains))
...
class AbstractMMD(ERM):
    ...
    def my_cdist(self, x1, x2):
        ...
        res = torch.addmm(x2_norm.transpose(-2, -1),
                          x1,
                          x2.transpose(-2, -1), alpha=-2).add_(x1_norm) # torch.addm(input, mat1, mat2), input is added to mat1 matrix multiplication mat2 
        return res.clamp_min_(1e-30)

    def gaussian_kernel(self, x, y, gamma=[0.001, 0.01, 0.1, 1, 10, 100,
                                           1000]):
        D = self.my_cdist(x, y)
        K = torch.zeros_like(D) # return a tensor filled with the scalar value 0, with the same size as input 
    ...
    def mmd(self, x, y):
        if self.kernel_type == "gaussian":
            ...
            cent_x = x - mean_x
            cent_y = y - mean_y
            cova_x = (cent_x.t() @ cent_x) / (len(x) - 1) # @ as matrix multiplication 
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for i in range(nmb):
            ...
            for j in range(i + 1, nmb):
                ...
                penalty += self.mmd(features[i], features[j]) # point-wise MMD, across classes
        objective /= nmb
        if nmb > 1:
            penalty /= (nmb * (nmb - 1) / 2)
